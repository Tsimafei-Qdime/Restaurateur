...
"INSERT INTO [Users] ([Name] ,[Phone] ,[Email] ,[Password] ,[RoleId]) OUTPUT Inserted.ID VALUES (@Name ,@Phone,@Email,@Password,@RoleId)";//sql запрос
        ...
        {
            var queryString = 
"Delete from Menu where UpdatedBy=@id;" +
"Delete from Reviews where CreatedBy = @id;"+
"Delete from Reviews where RecepientId = @id;"+
"Delete from Bookings where UpdatedBy = @id;"+
"Delete from Users where id = @id; ";//sql запрос
